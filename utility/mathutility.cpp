...
Point3D MathUtility::ComputeMatrixMultiPoint(Matrix4d Mt, Point3D point)
{
    /*
    |m1  m2  m3  m4 |   |x|
    |m5  m6  m7  m8 | * |y|
...
Point3D MathUtility::ComputeMatrixMultiPoint(Matrix3d Mt, Point3D point)
{
    /*
    |m1  m2  m3 |   |x|
    |m4  m5  m6 | * |y|
...
Vector3D MathUtility::ComputeMatrixMutiVector(Matrix3d Mt, Vector3D vec)
{
    Vector3D tar;
...
double MathUtility::GetTriangleArea(const Point3D& v1,  const Point3D& v2, const Point3D& v3)
{
    Vector3D a = v2 - v1;
    Vector3D b = v3 - v1;
    Vector3D c = v2 - v3;
...
Vector3D MathUtility::GetTriangleVertexGradient( Vector3D a,  Vector3D b)
{
    Vector3D high, c;
    double len2 = 0;
    a = a*(-1);
...
Matrix4d MathUtility::Rotation2Matrix(double angle_x, double angle_y, double angle_z, bool x, bool y, bool z)
{
    Matrix4d rotation_x;
    Matrix4d rotation_y;
    Matrix4d rotation_z;
...
Matrix4d MathUtility::Rotation2Matrix(double angle,int x,int y,int z)
{
...
Matrix4d MathUtility::Translate2Matrix(double x,double y,double z)
{
    Matrix4d result = Matrix4d::Identity();
...
Matrix4d MathUtility::Scale2Matrix(double x,double y,double z)
{
    Matrix4d result = Matrix4d::Identity();
 ...
 Matrix4d MathUtility::MatrixMultNum(const Matrix4d mat, double factor)
 {
     Matrix4d mt = mat*factor;
     return mt;
 }

 Vector3D MathUtility::getNormalOfTri(const Point3D p1,const Point3D p2,const Point3D p3)
 {
 ...
 double MathUtility::ComputeMeshVolume(MyMesh& desMesh)
 {
 ...
 void MathUtility::EvaluateVNormalByArea(MyMesh& desMesh,vector<Vector3D>& vNormal)
 {
     desMesh.request_face_normals();
     desMesh.request_vertex_normals();
     desMesh.update_normals();
     ...
     for (MyMesh::VertexIter vi = desMesh.vertices_begin(); vi != desMesh.vertices_end(); vi++)
     {
         ...
         for(MyMesh::VertexFaceIter vf_i = desMesh.vf_begin(vi); vf_i != desMesh.vf_end(vi); vf_i++)
         {
             ...
             double area = MathUtility::GetTriangleArea(triPoint[0],triPoint[1],triPoint[2]);
